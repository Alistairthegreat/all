...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			if (!App) ajax('grab.txt',Game.CheckUpdatesResponse);
		}
		...
		Game.grandmaNames=['Oli\'s Grandma'];
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (list.length==0)
			{
				if (Game.cookiesEarned<5) list.push(loc("You feel like making cookies. But nobody wants to eat your cookies. Thanks for choosing Oli\'s Corner!"));
				...
				else if (Game.cookiesEarned<100000) list.push(loc("Your cookies now play on Oli\'s Corner!"));
